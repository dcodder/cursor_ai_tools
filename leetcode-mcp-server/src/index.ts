...
import { getClient, requireAuth, AUTH_MSG, type Site } from "./leetcode.js";
...
server.registerTool(
  ...
  {
    description:
      "Get full problem details by title slug (description, constraints, examples, hints, code snippets, tags).",
    inputSchema: {
  ...
  async ({ titleSlug, site }) => {
    try {
      const { client } = getClient(site as Site);
      const slug = titleSlug.toLowerCase().replace(/\s/g, "-");
      const problem = await (client as { problem(slug: string): Promise<unknown> }).problem(slug);
...
server.registerTool(
  ...
  {
    description:
      "List problems with optional filters: difficulty, tags, category, keyword. Returns id, title, slug, difficulty, tags.",
    inputSchema: {
  ...
  async ({ difficulty, tags, category, limit, offset, keyword, site }) => {
    try {
      const { client } = getClient(site as Site);
      const filters: { difficulty?: string; tags?: string[] } = {};
      if (difficulty) filters.difficulty = difficulty;
      if (tags?.length) filters.tags = tags;
      const list = await (client as {
        problems(opts: {
          category?: string;
          offset?: number;
          limit?: number;
      ...
      if (keyword?.trim()) {
        ...
        questions = questions.filter((q: unknown) => {
          const o = q as { title?: string; titleSlug?: string };
          return (
            (o.title && String(o.title).toLowerCase().includes(k)) ||
...
server.registerTool(
  ...
  {
    description:
      "Searches for LeetCode problems based on multiple filter criteria: category, tags, difficulty, and keywords in problem titles. Returns matching problems with id, title, slug, difficulty, and tags.",
    inputSchema: {
      category: z
        .string()
        .optional()
...
server.registerTool(
  ...
  {
    description:
      "Retrieves today's LeetCode Daily Challenge problem with complete details: description, constraints, examples, hints, code snippets, tags, solution metadata, and challenge date/link.",
  ...
  async ({ site }) => {
    try {
      const { client } = getClient(site as Site);
      const c = client as {
        daily(): Promise<{ date?: string; link?: string; question?: { titleSlug?: string }; questionId?: string; titleSlug?: string }>;
        problem(slug: string): Promise<unknown>;
      };
      ...
      const titleSlug =
        (questionRef && "titleSlug" in questionRef && questionRef.titleSlug) ||
      ...
      const meta = daily as { date?: string; link?: string };
      const result = {
        date: meta?.date,
        link: meta?.link,
...
server.registerTool(
  ...
  async ({ username, site }) => {
    try {
      const { client } = getClient(site as Site);
      const profile = await (client as { user(username: string): Promise<unknown> }).user(username);
...
server.registerTool(
  ...
  {
    description:
      "Get recent public submissions for a user (global: max 20; China may return more). Use username 'me' with auth for own submissions on global.",
    inputSchema: {
      username: z.string(),
  ...
  async ({ username, limit, site }) => {
    try {
      const { client, hasAuth } = getClient(site as Site);
      const c = client as {
        recent_submissions(username: string, limit?: number): Promise<unknown[]>;
        whoami?(): Promise<{ username?: string }>;
        userStatus?(): Promise<{ userSlug?: string; username?: string }>;
      };
      let uname = username;
...
server.registerTool(
  ...
  {
    description:
      "Get all submissions for the authenticated user. Auth required. Global only; for China use get_user_submissions with your username.",
    inputSchema: {
  ...
  async ({ limit, offset, slug, site }) => {
    try {
      requireAuth(site as Site);
      const { client } = getClient(site as Site);
      if (site === "china") {
        return textResult(
          "On China site use get_user_submissions with your username, or problem_submissions per problem. get_my_submissions (full list) is supported for global site only.",
          true
        );
      }
      const submissions = await (client as {
        submissions(opts: { limit?: number; offset?: number; slug?: string }): Promise<unknown[]>;
...
server.registerTool(
  ...
  {
    description:
      "Get submission detail including code and verdict. Auth required for full detail.",
    inputSchema: {
  ...
  async ({ submissionId, site }) => {
    try {
      ...
      if (site === "china") {
        const detail = await (client as {
          submissionDetail(submissionId: string): Promise<unknown>;
        }).submissionDetail(id);
        return textResult(safeJson(detail));
      }
      const detail = await (client as { submission(id: number): Promise<unknown> }).submission(
        Number.isNaN(numId) ? parseInt(id, 10) : numId
      );
      return textResult(safeJson(detail));
    } catch (err) {
      return textResult(
...
server.registerTool(
  ...
  async ({ username, site }) => {
    try {
      const { client } = getClient(site as Site);
      const info = await (client as {
        user_contest_info(username: string): Promise<unknown>;
      }).user_contest_info(username);
...
server.registerTool(
  ...
  {
    description:
      "Creates or updates your note for a specific LeetCode problem. Auth required. Uses LeetCode's undocumented GraphQL mutation; may not be available on all sites.",
    inputSchema: {
  ...
  async ({ questionId, content, summary, site }) => {
    try {
      ...
      const { client } = getClient(site as Site);
      const c = client as {
        problem(slug: string): Promise<{ questionFrontendId?: string; titleSlug?: string }>;
        problems(opts: { category?: string; offset?: number; limit?: number }): Promise<{
          questions?: Array<{ questionFrontendId?: string; titleSlug?: string }>;
        }>;
        graphql?(body: { query: string; variables: Record<string, string> }): Promise<{ data?: unknown; errors?: Array<{ message?: string }> }>;
      };
      const idTrim = questionId.trim();
      let titleSlug: string;
      if (/^\d+$/.test(idTrim)) {
        const list = await c.problems({ category: "", offset: 0, limit: 3000 });
        const q = (list.questions ?? []).find(
          (p: { questionFrontendId?: string }) => String(p.questionFrontendId) === idTrim
        ) as { titleSlug?: string } | undefined;
        if (!q?.titleSlug) {
          return textResult(
            safeJson({ error: `No problem found with question ID "${questionId}"` }),
            true
          );
        }
      ...
      if (res?.errors?.length) {
        const msg = res.errors.map((e: { message?: string }) => e?.message ?? "Unknown error").join("; ");
        return textResult(
          safeJson({
...
server.registerTool(
  ...
  {
    description:
      "Updates an existing note for a LeetCode problem with new content or summary. noteId is the problem's question ID or title slug. Auth required. Uses same undocumented API as create_note.",
    inputSchema: {
  ...
  async ({ noteId, content, summary, site }) => {
    try {
      ...
      if (/^\d+$/.test(idTrim)) {
        ...
        if (!q?.titleSlug) {
          return textResult(
            safeJson({ error: `No problem found with note ID "${noteId}"` }),
            true
          );
        }
        titleSlug = q.titleSlug;
...
server.registerTool(
  ...
  {
    description:
      "Retrieves user notes for a specific LeetCode problem by question ID or title slug. Auth required. Returns your note(s) for that problem with optional pagination (skip/limit).",
    inputSchema: {
      questionId: z.string().describe("Question ID (e.g. 1) or title slug (e.g. two-sum) of the problem"),
  ...
  async ({ questionId, limit, skip, site }) => {
    try {
      ...
      const c = client as {
        ...
        problems(opts: { category?: string; offset?: number; limit?: number }): Promise<{ questions?: Array<{ questionFrontendId?: string; titleSlug?: string }> }>;
      };
      const idTrim = questionId.trim();
      let titleSlug: string;
      if (/^\d+$/.test(idTrim)) {
...
server.registerTool(
  ...
  {
    description:
      "Searches for user notes on LeetCode. Returns notes from problems you have attempted/solved; optional keyword filter, pagination (skip/limit), and sort order. Auth required.",
    inputSchema: {
  ...
  async ({ keyword, limit, skip, orderBy, site }) => {
    try {
      ...
      const { client } = getClient(site as Site);
      const c = client as {
        user_progress_questions?(filters: { skip: number; limit: number }): Promise<{
          totalNum?: number;
          questions?: Array<{ titleSlug?: string; title?: string }>;
        }>;
        problem(slug: string): Promise<{ title?: string; titleSlug?: string; note?: string | null }>;
      };
      if (!c.user_progress_questions) {
        return textResult(
          "search_notes is not supported for this site (user progress list not available).",
          true
        );
      }
      ...
      const results: Array<{ titleSlug: string; title: string; note: string }> = [];
...
server.registerTool(
  ...
  {
    description:
      "Get your note for a problem (auth required). Provide titleSlug to fetch that problem's note.",
    inputSchema: {
  ...
  async ({ titleSlug, site }) => {
    try {
      ...
      if (!titleSlug?.trim()) {
        return textResult(
          "Provide titleSlug to get your note for a specific problem (e.g. two-sum).",
          false
        );
      }
      const slug = titleSlug.toLowerCase().replace(/\s/g, "-");
      const problem = await (client as { problem(slug: string): Promise<{ note?: string }> }).problem(
        slug
      );
      const note = problem?.note ?? "";
      return textResult(
        safeJson({ titleSlug: slug, note: note || "(no note saved)" })
      );
    } catch (err) {
      return textResult(
        err instanceof Error ? err.message : String(err),
...
const GLOBAL_ORDER_MAP: Record<string, string> = {
  HOT: "hot",
  MOST_RECENT: "newest_to_oldest",
  MOST_VOTES: "most_votes",
};
const CN_ORDER_MAP: Record<string, string> = {
  DEFAULT: "DEFAULT",
...
server.registerTool(
  ...
  {
    description:
      "Retrieves a list of community solutions for a specific LeetCode problem. Supports filters (userInput, tagSlugs), pagination (limit, skip), and sort order (orderBy). Global: HOT, MOST_RECENT, MOST_VOTES. China: DEFAULT, MOST_UPVOTE, HOT, NEWEST_TO_OLDEST, OLDEST_TO_NEWEST.",
    inputSchema: {
  ...
  async ({ questionSlug, limit, skip, userInput, tagSlugs, orderBy, site }) => {
    try {
      const { client } = getClient(site as Site);
      const c = client as {
        graphql?(body: { query: string; variables: Record<string, unknown> }): Promise<{
          data?: { questionSolutions?: { totalNum?: number; solutions?: unknown[]; hasDirectResults?: boolean } };
      ...
      if (res?.errors?.length) {
        const msg = res.errors.map((e: { message?: string }) => e?.message ?? "Unknown").join("; ");
        return textResult(safeJson({ error: "API error", details: msg }), true);
      }
      const qs = (res as { data?: { questionSolutions?: { totalNum?: number; solutions?: unknown[] } } })?.data
        ?.questionSolutions;
...
server.registerTool(
  ...
  {
    description:
      "Retrieves the complete content of a specific community solution. Global: use topicId (from list_problem_solutions). China: use slug.",
    inputSchema: {
  ...
  async ({ topicId, slug, site }) => {
    try {
      if (site === "china") {
        if (!slug?.trim()) {
          return textResult(
            safeJson({ error: "For China site, slug is required to retrieve a solution." }),
            true
          );
        }
        const { client } = getClient(site as Site);
        const c = client as {
          graphql?(body: { query: string; variables: Record<string, unknown> }): Promise<unknown>;
        };
        if (!c.graphql) {
      ...
      if (!topicId?.trim()) {
        return textResult(
          safeJson({ error: "For Global site, topicId is required (get it from list_problem_solutions)." }),
          true
        );
      }
      ...
      const { client } = getClient(site as Site);
      const c = client as {
        graphql?(body: { query: string; variables: Record<string, number> }): Promise<{
          data?: { topic?: unknown };
          errors?: Array<{ message?: string }>;
        }>;
      };
      ...
      const topic = (res as { data?: { topic?: unknown } })?.data?.topic;
...
server.registerTool(
  ...
  {
    description:
      "Fetch official solution by ID (from problem.solution.id). May be paid-only; auth may be required.",
    inputSchema: {
      solutionId: z.string(),
      site: siteSchema,
    },
  ...
  async ({ solutionId, site }) => {
    try {
      const { client } = getClient(site as Site);
      const c = client as { graphql?(q: unknown): Promise<unknown> };
      if (!c.graphql) {
